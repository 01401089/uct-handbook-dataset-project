...
```
python run_pipeline.py --years all      # batch: fees -> all faculty extractors -> assemble -> validate -> final layer
```
...
python -m extractors.<fac>.extract --year YYYY   # com/ebe/law/fhs/sci/hum
python build_main_dataset.py      --year YYYY
python validation/validate.py     --year YYYY
python build_final_dataset.py     --year YYYY
python validation/validate_final.py --year YYYY
```
...
engine change re-run EVERY faculty on the engine and verify prior years'
rows are value-identical (the 2025 baseline especially). Writers are merge-by-year
...
in the faculty's register (`resolutions/<fac>.csv`) with rationale + page
evidence, applied only in the final layer. The finaliser fails on stale register entries; detectors R1b/
...
- **One extractor config per faculty.** Faculties present curricula
  ...
  general across COM, EBE and LAW (same publisher template) — faculty differences
  ...
  faculty whose books genuinely don't fit the template writes its own parser
  rather than bending the engine — FHS, SCI and HUM do exactly this, reusing
  the shared grammar and the engine's catalogue parser.
